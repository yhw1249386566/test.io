...
const TodoView = inject(
    ...
    (props: any) => {
        ...
        const handleDeleteClick = (todo, index) => { // 点击删除时，删除对应的待办
            removeTodo('todos', todo)
            removeTodo('todoscopy', todoscopy[index])
        }
        const todoItem = todos.map((todo, index) => (   // 渲染每条待办事项
            <li key={todo.id}>
                <TodoItem todo={todo} index={index} />
                <svg className={Style.deletetodoicon} aria-hidden="true" onClick={action(() => { handleDeleteClick(todo, index) })}>
...
const TodoItem = inject(
    ...
    (props: any) => {
        const { todo, index, todoscopy, todos, removeTodo, storageTodos } = props
        const [userChangeValue, setUserChangeValue] = useState('')
        const handleChange = (changValue: string) => setUserChangeValue(changValue) // React 代理受控组件
        ...
        const handleOnblur = (todo: any) => { // 用户输入框失去焦点时，删除空待办。
            removeTodo('todos', todo)
            removeTodo('todoscopy', todoscopy[index])
        }
        useEffect(() => { setUserChangeValue(todo.userInputValue); }, [1]); // 挂载完成时将已有数据渲染到每条输入框中
        useEffect(() => {// 实时更新用户输入值并存入本地（先更新后存）
        ...
        useEffect(() => { storageTodos('todoscopy', todoscopy) })
        const classNameChecbox = cx({ // 当复选框被勾选时，添加 class
        ...
        return (
            <>
                ...
                <textarea // 显示待办事项
                    ...
                    onKeyDown={(e) => e.key === 'Enter' ? e.preventDefault() : null} // 禁止回车
                    onBlur={!userChangeValue ? action(() => handleOnblur(todo)) : null} // 删除空待办
                />
...
const TodoTime = inject(allStores => ({
    ...
    (props: IPropsTodoTime) => {
        ...
        const classNameHourglass = cx({ // 设置差 1 小时 todo 即将过期时沙漏的 class
            isAboutToExpire: duetotime * 24 * 60 - todoCreatedTime <= 60,
            timehourglass: true,
        })
        const classNameTodoCreatedTime = cx({ // 设置即将到期和已过期的‘已创建时间’的颜色
            willExpire: duetotime * 24 * 60 - todoCreatedTime <= 60,
            expiredL: duetotime * 24 * 60 - todoCreatedTime <= 0,
            countdown: true,
        })
        const handleDueToTimeChange = (e: any) => setDueToTime(e?.target?.value?.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g, '')) // 只能输入数字
        const displayTodoCreatedTime = () => { // 显示 Todo 已创建时间
            ...
            todoCreatedTime * 60 > 0 && todoCreatedTime * 60 < 60 // 已创建时间 > 0 秒 且 < 60 秒 
                ? setShowTodoCreatedTime(`已创建 ${(todoCreatedTime * 60).toFixed(0)} 秒`)
                : todoCreatedTime < 60 // 创建时间是否 > 60 分钟
                    ? setShowTodoCreatedTime(`已创建 ${todoCreatedTime.toFixed(0)} 分钟`)
                    : todoCreatedTime < 1440  // 已创建时间是否大于 1 天
                        ? setShowTodoCreatedTime(`已创建 ${(todoCreatedTime / 60).toFixed(0)} 小时`)
                        : todoCreatedTime < 525600 // 创建时间是否大于 1 年
                            ? setShowTodoCreatedTime(`已创建 ${(todoCreatedTime / 60 / 24).toFixed(0)} 天`)
                            : setShowTodoCreatedTime(`已创建 ${(todoCreatedTime / 60 / 24 / 365).toFixed(0)} 年`)
        }
        const displayTodoDueToTime = () => {  // 显示 Todo 到期时间
            // 用户输入待办事项持续的天数（转为min） - 已创建时间  = 是否过期
            let dueTimeMin = duetotime * 24 * 60 - todoCreatedTime // 是否过期（min/h/s）指的是还剩总的多少 min/h/s 过期
            let [dueTimeHour, dueTimeDay, dueTiemYear] = [dueTimeMin / 60, dueTimeMin / 60 / 24, dueTimeMin / 60 / 24 / 365]
            dueTimeMin < 0 ? setShowIsDue('已过期')
                : dueTimeMin < 60 ? setShowIsDue(`还剩 ${dueTimeMin.toFixed(0)} 分钟到期`)
                    : dueTimeHour < 24 ? setShowIsDue(`还剩 ${dueTimeHour.toFixed(0)} 小时到期`)
                        : dueTimeHour < 525600 ? setShowIsDue(`还剩 ${dueTimeDay.toFixed(0)} 天到期`)
                            : setShowIsDue(`还剩 ${dueTiemYear.toFixed(0)} 年到期`)
        }
        useEffect(() => { displayTodoDueToTime() }) // 显示 todo 的到期时间
        // 显示 Todo 已创建时间
        useEffect(() => { displayTodoCreatedTime(); setInterval(() => { displayTodoCreatedTime() }, 5000) }, [1])
        useEffect(() => { setDueToTime(todo.duetotime) }, [1]) // 从本地取出到期时间渲染到页面
        useEffect(() => { todo.duetotime = duetotime }) // 每次用户更改到期时间时，就更新变量中对应的 todo（不需要更改 todoscopy[index]）
        useEffect(() => { storageTodos('todoscopy', todoscopy) }) // 将用户更改到期时间后的数据存入本地
        return <div className={Style.timer}> {/* 过输入的期时间 + 已创建时间 */}
            ...
            <div className={classNameHourglass} title={showIsDue} /> {/* 沙漏图片 */}
            <span className={classNameTodoCreatedTime}>{showTodoCreatedTime}</span> {/* 创建时间： */}
        </div>
    }
))

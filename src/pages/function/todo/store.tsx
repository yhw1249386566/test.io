...
class Store {
    ...
    get left() { return this.getCurrentUncompleteTask.length } // 得到当前未完成任务长度
    get getCurrentCompletedTask() { return this.todoscopy.filter((todocopy) => todocopy.finished === true) } // 得到 todoscopy 当前已完成任务
    get getCurrentUncompleteTask() { return this.todoscopy.filter((todocopy) => todocopy.finished === false) } // 得到 todoscopy当前未完成任务
    ...
    constructor() {
        this.createTodo = (arrName, userInputItem) => {
            userInputItem !== "" ? this[arrName].unshift(new Todo(userInputItem)) : null // 防止添加空待办
        }
        this.removeTodo = (deleteTodosName, todo) => this[deleteTodosName].remove(todo) // observable 的内置方法，参见：https://cn.mobx.js.org/refguide/array.html
        ...
        this.storageTodos = (localname, todos) => localStorage.setItem(localname, JSON.stringify(todos))

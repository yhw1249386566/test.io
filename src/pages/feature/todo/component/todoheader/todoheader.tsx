...
const TodoHeader = inject('store')(
    observer((props: any) => {
        // BUG：快速刷新页面(F5) 会导致第2次还未重新存储Todos到本地数据时，就获取本地数据，从而导致最终本地数据为空
        const currentLocalstorageTodosCopy =
            localStorage.getItem('todoscopy') ?? '[]' // 使用 setTImtout 模拟延迟时，需要先将数据存储到变量，然后使用变量作为数据来源，否则 setTimeout 中获取不到
        const { store } = props
        useEffect(() => {
            document
                .querySelector('#mask')
...
// logo+输入框+添加按钮+输入框阴影
const HeaderInputForm = inject('store')(
    observer((props: any) => {
        const { store } = props
        const [inputValueState, setInputValueState] = useState('')
        const handleChange = (inputValueState: any) =>
            setInputValueState(inputValueState) // 使 React 代理受控组件
        ...
        return (
            <form
                ...
                className={Style.todoheaderform}
            >
...
// 完成所有/全未完成+清空未完成 按钮
const ButtonAndLogo = inject('store')(
    observer((props: any) => {
        const { store } = props
        const _build = () =>
            // 渲染切换按钮 完成所有/全未完成
        ...
        useEffect(() => {
            store.storageTodos('todoscopy', store.todoscopy)
        })
        return (
            <div className={Style.todoheaderdiv}>
                <div className={Style.switchonoffbox}>{_build()}</div>
                <div className={Style.deleteallbox}>
                    <svg className={Style.deleteallicon} aria-hidden='true'>
                        <use xlinkHref='#icon-qingkong1' />
                    </svg>
                    <span onClick={action(() => store.clearFinished())}>
                        清空已完成
                    </span>
                </div>
            </div>
...
const CompleteAll = inject('store')(
    observer((props: any) => (
        <div>
            ...
            <span
                onClick={action(
                    () =>
                        (props.store.changeAllTodoListCompleted = [
                            'todos',
                            'todoscopy',
                        ]),
                )}
...
const AllUnfinish = inject('store')(
    observer((props: any) => (
        <div>
            ...
            <span
                onClick={action(
                    () =>
                        (props.store.changeAllTodoListUncompleted = [
                            'todos',
                            'todoscopy',
                        ]),
                )}
